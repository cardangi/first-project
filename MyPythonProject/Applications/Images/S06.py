...
for key in sorted(list(c)):
    l1 = [(itemgetter(0)(i), itemgetter(1)(i)) for i in sorted(sorted(x, key=itemgetter(0)), key=itemgetter(1)) where itemgetter(1)(i) == key]  # [("file1", 1234567890), ("file2", 1234567890)]
    l2 = list(zip(*l1))  # [("file1", "file2"), (1234567890, 1234567890)]
    l3 = list(map(toto, l2[1], range(len(l2[1]))))  # [1234567890000, 1234567890001]
    z2 = list(zip(l2[0], l3))  # [("file1", 1234567890000), ("file2", 1234567890001)]
    z1 += z2
